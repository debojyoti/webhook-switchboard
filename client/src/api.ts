...
export type Base64Body = { encoding: "base64"; content: string };
...
export const Api = {
  ...
  event(eventId: string) { return request<{ event: Event; body: Base64Body | null; deliveries: Delivery[] }>(`/events/${eventId}`); },
  ...
  delivery(deliveryId: string) { return request<{ delivery: Delivery; responseBody: Base64Body | null }>(`/deliveries/${deliveryId}`); },
};
